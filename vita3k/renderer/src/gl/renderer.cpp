...
void GLState::render_frame(const SceFVector2 &viewport_pos, const SceFVector2 &viewport_size, const DisplayState &display,
    ...
    const std::size_t texture_data_size = display.pitch * display.image_size.y * 4;
    ...
    for (std::size_t i = 0; i < gxm.surface_syncing_infoes.size(); i++) {
        ...
        if ((gxm.surface_syncing_infoes[i].addr <= display.base.address()) && (gxm.surface_syncing_infoes[i].addr + texture_data_size > display.base.address())) {
    ...
    if (check_for_cache)
        surface_handle = surface_cache.sourcing_color_surface_for_presentation(display.base, display.image_size.x, display.image_size.y, display.pitch, uvs);
    ...
    if (!surface_handle) {
        ...
        if (pixels) {
            open_access_parent_protect_segment(mem, display.base.address());
            unprotect_inner(mem, display.base.address(), texture_data_size);
        }
        ...
        if (pixels) {
            close_access_parent_protect_segment(mem, display.base.address());
        }
